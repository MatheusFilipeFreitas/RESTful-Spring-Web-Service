...
public class AmazonSES {
    ...
    // This address must be verified with Amazon SES (will send the email)
    ...
    final String PASSWORD_RESET_HTMLBODY = "<h1>A request to reset your password</h1>"
            + "<p>Hi, $firstName!</p> "
            + "<p>Someone has requested to reset your password with out project. If it were not you, please ignore this email"
            + " otherwise please click on the link below to set a new password: "
            + "<a href='http://localhost:8888/verification-service/password-reset.html?token=$tokenValue'>"
    ...
    final String PASSWORD_RESET_TEXTBODY = "A request to reset your password. "
            + "Hi, $firstName! "
            + "Someone has requested to reset your password with out project. If it were not you, please ignore this email "
            + " otherwise please open the link below in your browser window to set a new password: "
            + " http://localhost:8888/verification-service/password-reset.html?token=$tokenValue"
            + " Thank you!";
    ...
    public  boolean sendPasswordResetRequest(String firstName, String email, String token) {
        ...
        SendEmailRequest request = new SendEmailRequest().withDestination(new Destination().withToAddresses(email)).withMessage(new Message().withBody(new Body()
                                .withHtml(new Content().withCharset("UTF-8").withData(htmlBodyWithToken))
                                .withText(new Content().withCharset("UTF-8").withData(textBodyWithToken)))
